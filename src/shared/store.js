...
function convertFullUsernameToEnvFormat (fullUsername) {
  // gh/motdotla => GH_MOTDOTLA_DOTENVX_PRO_TOKEN
  return fullUsername
    .toUpperCase()
    .replace(/\//g, '_') // Replace all slashes with underscores
    .concat('_DOTENVX_PRO_TOKEN') // Append '_DOTENVX_PRO_TOKEN' at the end
}

function findFirstMatchingKey (data) {
  const dotenvxTokenValue = data.DOTENVX_PRO_TOKEN

  for (const [key, value] of Object.entries(data)) {
    if (key !== 'DOTENVX_PRO_TOKEN' && value === dotenvxTokenValue) {
      return key
    }
  }
...
function parseUsernameFromTokenKey (key) {
  // Remove the leading GH_/GL_ and trailing '_DOTENVX_PRO_TOKEN'
  const modifiedKey = key.replace(/^(GH_|GL_)/, '').replace(/_DOTENVX_PRO_TOKEN$/, '')
...
const getLatestVersion = function () {
  return confStore.get('DOTENVX_PRO_LATEST_VERSION') || packageJson.version
}

const getLatestVersionLastChecked = function () {
  return parseInt(confStore.get('DOTENVX_PRO_LATEST_VERSION_LAST_CHECKED') || 0)
}
...
const setLatestVersion = function (version) {
  confStore.set('DOTENVX_PRO_LATEST_VERSION', version)

  return version
}

const setLatestVersionLastChecked = function (dateNow) {
  confStore.set('DOTENVX_PRO_LATEST_VERSION_LAST_CHECKED', dateNow)
...
const deleteToken = function () {
  // memory user
  const key = findFirstMatchingKey(confStore.store) // GH_MOTDOTLA_DOTENVX_PRO_TOKEN
