...
const token = require('../../../../src/cli/actions/settings/token')
...
t.test('token', ct => {
  ...
  const stub1 = sinon.stub(store, 'getToken').returns('dxo_1234')
  ...
  t.ok(stub1.called, 'store.getToken() called')
...
t.test('token - undefined', ct => {
  const optsStub = sinon.stub().returns({})
  const fakeContext = { opts: optsStub }
  const stub1 = sinon.stub(store, 'getToken').returns(undefined)
  const processExitStub = sinon.stub(process, 'exit')
